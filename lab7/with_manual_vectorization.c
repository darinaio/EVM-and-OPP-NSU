...
float dot(const float* a, const float* b); // векторизована
...
void SubtractMatrix(Matrix* matrix, const Matrix* term); // векторизована
void AddMatrix(Matrix* matrix, const Matrix* term); //векторизована
void MultiplyByNumber(Matrix* matrix, float number);//векторизована
...
/* __m128 – это тип данных из xmmintrin.h, представляющий 128-битный вектор, содержащий четыре числа с плавающей точкой одинарной точности.
Приведение (void*)  предотвращает предупреждения компилятора о неявном преобразовании типов, 
а затем указатели reinterpret_cast'ятся к  __m128*,  чтобы рассматривать входные данные как массивы 128-битных векторов.*/

/*reinterpret_cast — это оператор преобразования типов,  который позволяет изменить интерпретацию битов в памяти, не изменяя сами биты.
Он берет указатель на один тип данных и преобразует его в указатель на другой тип данных,
при этом не выполняется никаких преобразований значений. 
Просто меняется *виденье* компилятором того, что находится по этому адресу в памяти.*/
...
/*mm_sub_ps - выполняет поэлементное вычитание двух векторов с плавающей точкой одинарной точности (ps - packed single).  
Каждый элемент первого вектора вычитается из соответствующего элемента второго вектора. Результат – новый вектор */

/*_mm_storeu_ps - записывает 128-битный вектор в память. 
u в названии означает *unaligned*,  то есть функция может записывать данные по невыровненному адресу */
